...
router.post('/add-book', upload.single('image'), [
    body('name').isLength({ min: 3 }).withMessage('name should be atleast 3 charechters long'),
    body('description').isLength({ min: 5 }).withMessage('description should be atleast 3 charechters long')
...
router.patch('/update-book/:id', [
    ...
    try {
        ...
        if (!book) return res.status(404).send({ error: 'no book found!' })
        if (req.user._id.toString() !== book.authorId.toString()) return res.status(401).send({ error: 'Please authintcate!' })

        req.body.name ? book.name = req.body.name : book.name = book.name
        req.body.type ? book.type = req.body.type : book.type = book.type
        req.body.description ? book.description = req.body.description : book.description = book.description
...
router.get('/my-books', isAuth, async (req, res) => {
    try {
        const user = await User.findById({ _id: req.user._id }).populate('books.bookId')
...
router.get('/book-by-name', async (req, res) => {
    try {
        ...
        if (!bookName) return res.status(400).send({ error: 'please provide a book name' })
        const books = await Book.find({ name: bookName })
            .sort({ createdAt: 1 })
            .limit(parseInt(req.query.limit))
            .skip(parseInt(req.query.skip))
        if (!books.length) return res.status(404).send({ error: 'No book with this name found' })
        res.send(books)
    } catch (err) {
        res.send(err)
    }
